...
## The app never opens to a blank screen
...
has nothing to show and no promise can change that. The obligation is to survive every load after
one successful visit — and, per [../constraints.md](../constraints.md), a home-screen install starts
with an empty store, so an installed app's own first launch is a first visit again.
...
[What can a player do with no network?](../questions/what-can-a-player-do-with-no-network.md) sets
how much of the app is covered — this promise is the floor beneath that answer rather than a
substitute for it.
