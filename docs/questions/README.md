...
1. **A server answers one route, observed with curl, locally.**
   ...
   - **Given:** [0024-the-entry-document-is-a-build-output-not-a-per-request-render](../decisions/0024-the-entry-document-is-a-build-output-not-a-per-request-render.md) — so nothing forces a meta-framework's server here, and nothing excludes one either: the questions below choose on their own merits
     - **Must answer:** [what-horizon-is-this-built-for](what-horizon-is-this-built-for.md) — or else the runtime is scored on stability against a duration nobody stated, and the two arguments that duration licenses point opposite ways. Costs nothing to answer and invalidates the runtime record if it lands after it
     - **Must answer:** [what-must-a-dependencys-stewardship-satisfy](what-must-a-dependencys-stewardship-satisfy.md) — or else the runtime field's one immature survivor is dropped at spike time on taste rather than on a property, and the record inherits an elimination nobody argued. Costs a re-scaffold if the criterion later reopens the field
...
2. **A browser shows "Hello!" rendered by the client, locally.**
   ...
   - **Given:** [a-device-too-old-to-run-the-app-is-told-so-rather-than-shown-a-blank-screen](../guarantees/a-device-too-old-to-run-the-app-is-told-so-rather-than-shown-a-blank-screen.md) — the entry document carries a fallback the bundle cannot deliver, because a browser below the floor never runs it
     - **Must answer:** [what-horizon-is-this-built-for](what-horizon-is-this-built-for.md) — or else the renderer is scored on stability against a duration nobody stated, and this is the slice where that scoring actually eliminates candidates. Costs nothing to answer and invalidates the renderer record if it lands after it
     - **Must answer:** [what-must-a-dependencys-stewardship-satisfy](what-must-a-dependencys-stewardship-satisfy.md) — or else three of this slice's eliminations rest on a property no document supports, one of which has already been found wrong on its facts. Costs the largest re-scaffold M1 can create, because reopening the renderer field after the client is written rewrites the client half
...
- **Stewardship is a criterion nobody chose, and it is already eliminating candidates.** The property
  list declined to write a maintenance property because nothing in the repository supports one, and
  three question files use one anyway. It is now asked at
  [what must a dependency's stewardship satisfy?](what-must-a-dependencys-stewardship-satisfy.md),
  which rests on [what horizon is this built for?](what-horizon-is-this-built-for.md) because "well
  maintained enough" needs a duration to be enough for. Both sit between phase 3 and phase 4 of the
  plan above: verification establishes what is true about each candidate, and this decides which of
  those truths disqualify anything. Answering the spike's candidate list without them is where the
  unargued elimination gets made.
